...
public class SyncService extends IntentService {
    ...
    final String BASE_URL = "http://opensource-forever.com/final/list.php"; //Mark
...
//                    "http://opensource-forever.com/final/list.php";
            ...
            Log.e(LOG_TAG, "^^^^^^^^^^^^^ connection failed ^^^^^^^^^^^^^^^^^^^^^Error ", e);
            e.printStackTrace();
    ...
    private void updateSQLite(String str) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            try {
                ...
                values.put(MembersProvider.COLUMN_DISPLAY_NAME, displayName);
